...
const flatten = obj => ({
  ...
  dmode: 0o777 & (~obj.umask),
  fmode: 0o666 & (~obj.umask),
  umask: obj.umask,
  ...
  // the noProxy conf should be comma-separated so it matches the env
  // yes its weird, but this is how make-fetch-happen expects it to be set.
  noProxy: obj.noproxy.join(','),
...
const flatOptions = npm => npm.flatOptions ||
  Object.freeze(flatten(npm.config.list[0]))
