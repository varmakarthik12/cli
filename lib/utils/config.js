...
// Each of these definitions MUST have at minimum a description, type,
// and default set.
// If the defaultDescription or typeDescription are not set, they will
// be generated programmatically from the default or type, respectively.
const definitions = {
  _auth: {
    default: null,
    type: [null, String],
  ...
  color: {
    default: process.env.NO_COLOR == null,
    type: ['always', Boolean],
    description: `
...
const unindent = s => s.trim().replace(/\n      /g, '\n').trim()

const wrap = (s) =>
  unindent(s).split(/[ \n]+/).reduce((left, right) => {
    const last = left.split('\n').pop()
    const join = last.length && last.length + right.length > 70 ? '\n' : ' '
    return left + join + right
  })

const describe = key => {
  const def = definition(key)
  if (!def.defaultDescription)
    def.defaultDescription = describeValue(def.default)
  if (!def.typeDescription)
    def.typeDescription = describeType(def.type)
  const description = unindent(def.description)
  const deprecated = !def.deprecated ? ''
    : `* DEPRECATED: ${wrap(def.deprecated).replace(/\n/g, '\n  ')}\n`
  return `#### ${key}

* Default: ${wrap(def.defaultDescription).replace(/\n/g, '\n  ')}
* Type: ${wrap(def.typeDescription).replace(/\n/g, '\n  ')}
${deprecated}
${description}
`
}
...
if (module === require.main)
  console.log(process.argv[2] ? describe(process.argv[2]) : describeAll())
