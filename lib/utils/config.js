...
const COLS = Math.min(process.stdout.columns || 80, 80) - 5
const wrap = (s) => unindent(s).split(/[ \n]+/).reduce((left, right) => {
...
const describe = key => {
  ...
  const deprecated = !def.deprecated ? ''
    : `* DEPRECATED: ${unindent(def.deprecated)}\n`
  return wrapAll(`#### ${key}

* Default: ${unindent(def.defaultDescription)}
* Type: ${unindent(def.typeDescription)}
${deprecated}
${description}
`)
}
