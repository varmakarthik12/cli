...
define('_auth', {
  ...
  description: `
    ...
    is safer to use a registry-provided authentication bearer token by
    running \`npm login\`.
  `,
})
...
define('also', {
  ...
  flatten: (key, value, obj, flatOptions) => {
    if (obj.omit && /^dev(elopment)$/.test(value))
      obj.omit = obj.omit.filter(type => type !== 'dev')
  },
})
...
define('cache', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    flatOptions.cache = join(value, '_cacache')
  },
})
...
define('cafile', {
  ...
  `,
  // XXX: handled by @npmcli/config, but should really flatten it here
  // instead.  Need the flatten functions to support returning a promise,
  // or just read it using readFileSync.
})
...
define('description', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    flatOptions.search.description = value
  }
})
...
define('fetch-retries', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    flatOptions.retry.retries = value
  }
})
...
define('fetch-retry-factor', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.retry = flatOptions.retry || {}
    flatOptions.retry.factor = value
  }
})
...
define('fetch-retry-maxtimeout', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.retry = flatOptions.retry || {}
    flatOptions.retry.maxTimeout = value
  }
})
...
define('fetch-retry-mintimeout', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.retry = flatOptions.retry || {}
    flatOptions.retry.minTimeout = value
  }
})
...
define('include', {
  ...
  flatten (key, value, obj, flatOptions) {
    if (flatOptions.omit)
      flatOptions.omit = flatOptions.omit.filter(type => !value.includes(type))
  }
})
...
define('noproxy', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    flatOptions.noProxy = value.join(',')
  },
})
...
define('omit', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    flatOptions.omit = omit.concat(value).filter(type => !include.includes(type))
  }
})
...
define('only', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    if (include.includes('dev'))
      return
    flatOptions.omit = flatOptions.omit || []
    flatOptions.omit.push('dev')
  },
})
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    if (!value) {
      if (include.includes('optional'))
        return
      const omit = flatOptions.omit || []
      flatOptions.omit = omit.concat('optional')
    } else if (flatOptions.omit && flatOptions.omit.includes('optional'))
      flatOptions.omit = flatOptions.omit.filter(t => t !== 'optional')
  },
})
...
define('package-lock', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    flatOptions.packageLock = !(obj['package-lock'] === false ||
...
define('production', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    if (include.includes('dev'))
      return
    const omit = flatOptions.omit || []
    flatOptions.omit = omit.concat('dev')
  }
})
...
define('save-bundle', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    flatOptions.saveBundle = value && !obj['save-peer']
  }
})
...
define('save-dev', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    // for historical reasons, peer, optional, and peerOptional take
    // precedence over --save-dev
...
define('save-optional', {
  ...
  flatten (key, value, obj, flatOptions) {
    if (flatOptions.saveType === 'peerOptional')
      return
    else if (flatOptions.saveType === 'peer')
      flatOptions.saveType = 'peerOptional'
    else
      flatOptions.saveType = 'optional'
  },
...
define('save-peer', {
  ...
  flatten (key, value, obj, flatOptions) {
    if (flatOptions.saveType === 'peerOptional')
      return
    else if (flatOptions.saveType === 'optional')
      flatOptions.saveType = 'peerOptional'
    else
...
define('scope', {
  ...
  `,
  flatten (key, value, obj, flatOptions) {
    flatOptions.projectScope = value
  }
})
...
define('searchexclude', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.search = flatOptions.search || { limit: 20 }
    flatOptions.search.exclude = value
  },
})
...
define('searchlimit', {
  ...
  flatten (key, value, obj, flatOptions) {
    ...
    flatOptions.search.limit = value
  },
})
...
define('searchopts', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.search = flatOptions.search || { limit: 20 }
    flatOptions.search.opts = querystring.parse(value)
  },
})
...
define('searchstaleness', {
  ...
  flatten (key, value, obj, flatOptions) {
    flatOptions.search = flatOptions.search || { limit: 20 }
    flatOptions.search.staleness = value
  },
})
