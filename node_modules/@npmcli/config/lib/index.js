...
class Config {
  ...
  constructor ({
    ...
    if (this.list[0] !== this.data.get('cli').data) {
      throw new Error('list[0] is not cli??')
    }
  ...
  // create/update the flattened data object passed to dependencies
  flatten () {
    this.flat = {}
    // walk from least priority to highest
    for (const { data } of this.data.values()) {
      this[_flatten](data, this.flat)
    }
    return this.flat
  }
  ...
  invalidHandler (k, val, type, source, where) {
    ...
    if (Array.isArray(type)) {
      if (type.include(typeDefs.url.type))
